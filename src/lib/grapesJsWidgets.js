 ...
 */
import { getAvailableWidgets, getWidgetDefaultData, loadPublicWidget } from './utils';
    ...
    for (const category of widgetCategories) {
      for (const widget of category.items) {
        ...
        blockManager.add(`widget-${id}`, {
          ...
          media: thumbnail || `<svg viewBox="0 0 24 24"><path d="M22 9L12 5 2 9 12 13 22 9zM12 3L1 8l11 5 11-5-11-5z"></path><path d="M2 11v8l10 5 10-5v-8"></path></svg>`,
        });
      }
    }
  } catch (error) {
    ...
    model: {
      ...
      async render() {
        try {
          ...
          const templateId = this.get('templateId') || attributes['data-widget-template'];
          const widgetId = this.get('widgetId') || attributes['data-widget-id'] || `${folder}_${templateId}`;
          ...
          this.components(`<div class="widget-error">Error rendering widget: ${error.message}</div>`);
        }
      }
    },
