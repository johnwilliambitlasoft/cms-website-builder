...
const WidgetCustomizer = ({ widget, data, onChange }) => {
                    ...
                    {field.itemFields.map(itemField => (
                      <div key={itemField.id} className={styles.formGroup}>
                        <label htmlFor={`${field.id}-${index}-${itemField.id}`}>
                          {itemField.label}
                          {itemField.required && <span className={styles.required}>*</span>}
                        </label>
                        {renderItemField(field.id, index, itemField, item[itemField.id] || '')}
                      </div>
                    ))}
                  </div>
                </div>
      ...
      case 'text':
        return (
          <input
            ...
            onChange={(e) => handleArrayItemChange(arrayId, itemIndex, itemField.id, e.target.value)}
            className={styles.textInput}
  ...
  return (
    <div className={styles.widgetCustomizer}>
      <h3 className={styles.widgetTitle}>{widget.title || 'Widget Settings'}</h3>
